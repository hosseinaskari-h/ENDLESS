...
export default class Boid {
  ...
  getBoid(position = new THREE.Vector3(0, 0, 0), quaternion = null, color = 0x156289) {
    ...
    var sphereMesh = new THREE.Mesh(sphereGeometry, meshMaterial);

    // Create line to the center
    var lineGeometry = new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(0, 0, 0), position]);
    var line = new THREE.Line(lineGeometry, lineMaterial);

    // Create a group and add sphere and line
...
context.fillText(position.x.toFixed(2) + ', ' + position.y.toFixed(2) + ', ' + position.z.toFixed(2), canvas.width / 2, canvas.height / 2);

// Create texture from canvas
var texture = new THREE.CanvasTexture(canvas);
var spriteMaterial = new THREE.SpriteMaterial({ map: texture });
var textSprite = new THREE.Sprite(spriteMaterial);
textSprite.scale.set(10, 10, 1); // Adjust scale
textSprite.position.set(0, 10, 0); // Adjust position
mesh.add(textSprite);
