...
import BoidColumns from './src/BoidColumns.js';
...
function init() {
  ...
  if (hasObstacles) {
    ...
    utils.addObstacle(obstacles, scene, 50, 50, 50, 0x553555, 150, -200, 200)
    utils.addObstacle(obstacles, scene, 50, 100, 100, 0x555455, 80, -100, -180)
    utils.addObstacle(obstacles, scene, 100, 50, 100, 0x555255, -220, -150, 180)
    utils.addObstacle(obstacles, scene, 100, 50, 50, 0x551555, -150, -150, -150)
    utils.addObstacle(obstacles, scene, 100, 50, 100, 0x557555, 20, -300, -20)
  }
  ...
  boidColumns = new BoidColumns(scene, boidManager.boids);}
...
function render() {
  ...
  if (!paused) {
    update(delta);
    boidColumns.update(); // Update the columns each frame
  }
  composer.render();
}
